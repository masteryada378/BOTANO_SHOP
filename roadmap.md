...
**Статус:** ✅ Прийнято (з 1 маленьким “але”)

**Що зроблено правильно:**

- Є всі обов’язкові секції: prerequisites, env, quick start, ports/urls, troubleshooting.
- Чітко підсвічено, що потрібен **`docker compose` v2** (це важливо через `service_healthy`).
- Табличка env для `backend/.env` дуже вдала: `DB_HOST=db` пояснено, і це збереже нерви.
- Є попередження про trailing slash для `VITE_API_URL` — це дрібниця, яка часто ламає запити.

**Зауваження (поправ прямо зараз, 1 рядок):**

- У troubleshooting ти пропонуєш `curl http://localhost:5005/api/cards`, але в поточному backend роут — **`/cards`**, а `api`-префікс є лише у `/api/health`.
    - Замінити на один із варіантів (обери один і тримай консистентно):
        - `curl http://localhost:5005/api/health`
        - `curl http://localhost:5005/cards`
...
1. **Створи файл компонента**:
    - рекомендований шлях: `frontend/src/layouts/BottomNavigation.tsx`  
      (бо `Header` уже в `layouts`, тримаємо консистентність)
2. **Визнач структуру пунктів меню** як типізований масив (без `any`):
    - 5 пунктів: `Home`, `Catalog`, `Cart`, `Wishlist`, `Profile`
    - Для кожного пункту: `to` (route), `label` (текст), `ariaLabel` (для a11y), `icon` (ReactNode або компонент-іконка)
3. **Реалізуй UI (Tailwind, mobile-first)**:
    - Верстка: 5 рівних елементів у ряд (`grid grid-cols-5` або `flex` з `justify-between`)
    - Кожен пункт: іконка + підпис, достатній tap-area (мінімум ~44px по висоті)
    - Стан active/hover/focus: виразний, але без “новорічної ялинки”
    - Підтримка safe-area: додай padding з урахуванням `env(safe-area-inset-bottom)` (щоб на iPhone не було болю)
4. **Інтеграція з роутером (поки лише всередині компонента):**
    - Використай `NavLink` з `react-router-dom`, щоб active-стилі були автоматичні.
    - Не придумуй state руками — роутер уже все знає.
5. **Badge для Cart (поки тільки API-заготовка):**
    - Підготуй місце під badge з кількістю (компонент/елемент).
    - Дані з контексту підтягнемо в Task #8 (Header badge) і/або коли підключатимемо навігацію — зараз можна зробити це як проп `cartItemsCount?: number` (не обов’язково передавати поки що).

**Файли для створення/змін:**

- `frontend/src/layouts/BottomNavigation.tsx` (create)

**Критерії приймання:**

- Компонент існує, **типізований**, без `any`.
- Має 5 пунктів меню (routes можуть бути “майбутні”, але компонент не підключений — тож нічого не ламається).
- Є a11y мінімум: `aria-label`/логічні підписи, фокус-стили не вбиті.
- Tailwind only, без `position: absolute` (і без `fixed` — це буде вирішено в наступному таску при підключенні).
